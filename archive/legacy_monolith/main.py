...
def filetypes_dir(path: str) -> tuple[str, str] | tuple[
...
def add_combobox(tab: ttk.Labelframe, text: tk.StringVar | str, col: int, row: int, width: int) -> ttk.Combobox:
    """
...
class Window(ttk.Frame):
    ...
    def __init__(self, master: tk.Tk):
        ...
        self.signal_generator_instance: tk.StringVar
        self.osc_inst: tk.StringVar
        self.powermeter_inst: tk.StringVar
        self.rf_gen_inst: tk.StringVar
    ...
    def create_cycling_axes(self):
        ...
        self.fig_cycling = create_figure(num=10, figsize=(8.5, 6))
    ...
    def create_main_menu(self):
        ...
        add_button(main_frame, "Time domain Power", self.open_time_domain_power_window, 1, 4, style='large.TButton')
        add_button(main_frame, "Exit", self._quit, 1, 5, style='large.TButton')
        # Add other buttons here for other windows in the future

    def open_s3p_window(self):
        """Open the S3P files window."""
        s3p_window = tk.Toplevel(self)
        s3p_window.title("S3P Files")
        self.setup_s3p_display_tab(s3p_window)

    def open_s2p_window(self):
        """Open the S2P files window."""
        s2p_window = tk.Toplevel(self)
        s2p_window.title("S2P Files")
        self.setup_s2p_display_tab(s2p_window)

    def open_pull_in_window(self):
        """Open the Pull-in files window."""
        pull_in_window = tk.Toplevel(self)
        pull_in_window.title("Pull-in Files")
        self.setup_pull_in_display_tab(pull_in_window)
    ...
    def open_resource_page_window(self):
        ...
        self.setup_scpi_configuration(resource_page_window)
    ...
    def open_pulsed_pull_in_window(self):
        ...
        self.setup_pull_in_measurement_pulsed(pulsed_pull_in_window)

    def open_time_domain_power_window(self):
        """Open the Time domain power measurement test window."""
        time_domain_power_measurement_window = tk.Toplevel(self)
        time_domain_power_measurement_window.title("Time domain power measurement test")
        self.setup_time_domain_measurement_window(time_domain_power_measurement_window)

    def setup_pull_in_measurement_pulsed(self, window):
        # This TAB is for Pull down voltage vs isolation measurement
        frame_test_pulsed_pull_in_comp_info = add_label_frame(window,
                                                              frame_name='Component information', col=0,
                                                              row=0)
        frame_test_pulsed_pull_in_signal_generator = add_label_frame(tab=window,
                                                                     frame_name='Signal Generator',
                                                                     col=0,
                                                                     row=1)
        frame_test_pulsed_pull_in_gen_controls = add_label_frame(tab=window,
                                                                 frame_name='General controls',
                                                                 col=2,
                                                                 row=0)
        frame_test_pulsed_osc_pull_in = add_label_frame(tab=window,
                                                        frame_name='Oscilloscope Tecktronix',
                                                        col=1, row=0, row_span=3)
        frame_test_pulsed_pull_in_measurement = add_label_frame(tab=window,
    ...
    def setup_cycling_measurement(self, window):
        ...
        frame_oscilloscope = add_label_frame(tab=window, frame_name='Oscilloscope', col=0, row=2)
        frame_signal_generator = add_label_frame(tab=window, frame_name='Signal Generator', col=0, row=1)
        frame_cycling_monitor = add_label_frame(tab=window, frame_name='Cycling monitor', col=1, row=0,
                                                row_span=4)
        ...
        add_button(tab=frame_signal_generator, button_name='1kHz-1000pulses\n100us-5%',
                   command=lambda: [scripts_and_functions.load_pattern(
                       r"1000pulses_100us_pulse_dc5.0%_36Vtop40V_triangle.arb")], col=0,
                   row=3).grid(
            ipadx=tab_pad_x, ipady=tab_pad_x)
        add_button(tab=frame_signal_generator, button_name='2kHz-1000pulses\n100us-10%',
                   command=lambda: [scripts_and_functions.load_pattern(
                       r"1000pulses_100us_pulse_dc10.0%_36Vtop40V_triangle.arb")], col=1,
                   row=3).grid(
            ipadx=tab_pad_x, ipady=tab_pad_x)
        add_button(tab=frame_signal_generator, button_name='4kHz-1000pulses\n100us-20%',
                   command=lambda: [scripts_and_functions.load_pattern(
                       r"1000pulses_100us_pulse_dc20.0%_36Vtop40V_triangle.arb")], col=2,
                   row=3).grid(
            ipadx=tab_pad_x, ipady=tab_pad_x)
    ...
    def setup_power_measurement(self, window):
        ...
        frame_power_meas = add_label_frame(tab=window, frame_name='General controls', col=2, row=0)
        frame_power_meas_graph = add_label_frame(tab=window, frame_name='Power test Graph', col=1, row=0,
                                                 row_span=3)
        frame_power_measurement_signal_generator = add_label_frame(tab=window,
                                                                   frame_name='Signal Generator', col=0, row=1,
                                                                   row_span=1)
        frame_power_meas_rf_gen = add_label_frame(tab=window, frame_name='RF Generator', col=2, row=1,
                                                  row_span=2)
        frame_power_meas_powermeter = add_label_frame(tab=window, frame_name='Powermeter', col=0, row=2,
                                                      row_span=1)
    ...
    def setup_snp_measurement(self, window):
        frame_snp_compo_info = add_label_frame(window, frame_name='Component information', col=0,
                                               row=0)  # s3p frame
        frame_snp_signal_generator = add_label_frame(tab=window, frame_name='Signal Generator', col=0, row=1)
        frame_snp_zva = add_label_frame(tab=window, frame_name='ZVA', col=3, row=1)
        frame_snp_gene_controls = add_label_frame(tab=window, frame_name='General controls', col=3, row=0)
        frame_snp_measurement = add_label_frame(tab=window, frame_name='SNP measurement', col=1, row=0,
                                                row_span=2)
        ...
        add_button(tab=frame_snp_gene_controls, button_name='Reset Signal Gen',
                   command=lambda: [scripts_and_functions.configuration_sig_gen_snp()],
                   col=1,
                   row=2).grid(ipadx=tab_pad_x, ipady=tab_pad_x)
    ...
    def setup_pull_in_measurement(self, window):
        ...
        frame_test_pull_in_comp_info = add_label_frame(window, frame_name='Component information', col=0,
                                                       row=0)
        frame_test_pull_in_signal_generator = add_label_frame(tab=window, frame_name='Signal Generator',
                                                              col=0,
                                                              row=1)
        frame_test_pull_in_gen_controls = add_label_frame(tab=window, frame_name='General controls',
                                                          col=2,
                                                          row=0)
        frame_osc_pull_in_test = add_label_frame(tab=window, frame_name='Oscilloscope Tecktronix',
                                                 col=1, row=0, row_span=3)
        frame_test_pull_in_measurement = add_label_frame(tab=window, frame_name='Measurement', col=2,
                                                         row=1,
                                                         row_span=2)
    ...
    def setup_pull_in_display_tab(self, window):
        ...
        self.slider_isolation = add_slider(frame=frame_v_pull_in_graph, _from=0, to=-20,
                                           name="Detector voltage (V)", variable=self.scale_isolation_value,
                                           step=1,
    ...
    def setup_time_domain_measurement_window(self, window):
        frame_time_domain_power_measurement_info = add_label_frame(
            window,
            'Time domain power measurement Directory', 0,
            0)  #

        frame_time_domain_power_measurement_display = add_label_frame(window,
                                                                      frame_name='Time domain power measurement Display',
                                                                      col=1, row=0)
        frame_time_domain_power_measurement_signal_generator = add_label_frame(tab=window,
                                                                               frame_name='Signal Generator', col=0,
                                                                               row=1,
                                                                               row_span=1)
        frame_time_domain_power_meas_rf_gen = add_label_frame(tab=window, frame_name='RF Generator', col=2, row=2,
                                                              row_span=1)
        frame_time_domain_power_meas_powermeter = add_label_frame(tab=window, frame_name='Powermeter', col=0, row=3,
                                                                  row_span=1)
        frame_time_domain_power_measurement = ttk.Frame(frame_time_domain_power_measurement_display)
        frame_time_domain_power_measurement.pack(anchor='nw')

        frame_time_domain_power_measurement_info.grid_columnconfigure(1, weight=1)
        frame_time_domain_power_measurement_signal_generator.grid_columnconfigure(2, weight=1)
        frame_time_domain_power_meas_rf_gen.grid_columnconfigure(3, weight=1)
        frame_time_domain_power_meas_powermeter.grid_columnconfigure(4, weight=1)

        add_label(frame_time_domain_power_measurement_info, label_name='DIR', col=0, row=0).grid(sticky='e',
                                                                                                 ipadx=tab_pad_x,
                                                                                                 ipady=tab_pad_x)
        add_label(frame_time_domain_power_measurement_info, label_name='Project', col=0, row=1).grid(sticky='e',
                                                                                                     ipadx=tab_pad_x,
                                                                                                     ipady=tab_pad_x)
        add_label(frame_time_domain_power_measurement_info, label_name='Cell', col=0, row=2).grid(sticky='e',
                                                                                                  ipadx=tab_pad_x,
                                                                                                  ipady=tab_pad_x)
        add_label(frame_time_domain_power_measurement_info, label_name='Reticule', col=0, row=3).grid(sticky='e',
                                                                                                      ipadx=tab_pad_x,
                                                                                                      ipady=tab_pad_x)
        add_label(frame_time_domain_power_measurement_info, label_name='Device', col=0, row=4).grid(sticky='e',
                                                                                                    ipadx=tab_pad_x,
                                                                                                    ipady=tab_pad_x)
        add_label(frame_time_domain_power_measurement_info, label_name='Status', col=0, row=5).grid(sticky='e',
                                                                                                    ipadx=tab_pad_x,
                                                                                                    ipady=tab_pad_x)
        add_label(frame_time_domain_power_measurement_info, label_name='Bias Voltage', col=0, row=6).grid(sticky='e',
                                                                                                          ipadx=tab_pad_x,
                                                                                                          ipady=tab_pad_x)
        add_label(frame_time_domain_power_meas_powermeter, label_name='Input Attenuation (A)', col=0, row=0).grid(
            sticky='e', ipadx=tab_pad_x, ipady=tab_pad_x)
        add_label(frame_time_domain_power_meas_powermeter, label_name='Output Attenuation (B)', col=0, row=1).grid(
            sticky='e', ipadx=tab_pad_x, ipady=tab_pad_x)
        add_label(frame_time_domain_power_meas_powermeter, label_name='(dB)', col=2, row=0).grid(
            sticky='e', ipadx=tab_pad_x, ipady=tab_pad_x)
        add_label(frame_time_domain_power_meas_powermeter, label_name='(dB)', col=2, row=1).grid(
            sticky='e', ipadx=tab_pad_x, ipady=tab_pad_x)

        self.time_domain_test_pow_dir = tk.StringVar(
            value=r'C:\Users\TEMIS\Desktop\TEMIS MEMS LAB\Measurement Data\Power handling\Time domain')
        self.time_domain_test_pow_project = tk.StringVar(value=r'Project_Name')
        self.time_domain_test_pow_cell = tk.StringVar(value=r'Cell_Name')
        self.time_domain_test_pow_reticule = tk.StringVar(value=r'Reticule')
        self.time_domain_test_pow_device = tk.StringVar(value=r'Device_name')
        self.time_domain_test_pow_file_created = tk.StringVar(value=r'EMPTY')
        self.time_domain_test_pow_state = tk.StringVar(value=r'EMPTY')
        self.time_domain_bias_voltage_pow = tk.StringVar(value=r'Bias_Voltage')
        self.time_domain_test_pow_input_atten = tk.DoubleVar(value=0)
        self.time_domain_test_pow_output_atten = tk.DoubleVar(value=0)
    ...
    def reset_signal_generator(self):  # Reset signal_generator using the IP address at Ressource Page (used in TAB4)
        ...
        address = self.signal_generator_instance.get()
        print(address, end='\n-----------------------------------------------')
    ...
    def acquire_pull_down_data_pulsed(
        ...
        #     filename=self.text_file_name_pull_in_test.get(index1="1.0", index2="end-1c"))
        # scripts_and_functions.print_error_log()
        self.set_txt(text_widget=self.text_gen_controls_pull_in_debug)
        # except:
        #     print("Error")
    ...
    def set_bias_voltage(self):
        ...
        scripts_and_functions.bias_voltage(str(bias))
    ...
    def set_bias_pull_in(
        ...
        scripts_and_functions.bias_pull_in_voltage(str(bias))
        self.error_log(scripts_and_functions.signal_generator)
    ...
    def set_txt(self, text_widget: tk.Text):  # (used in TAB5)
        # text_widget.delete(index1="%d.%d" % (1, 0), index2="%d.%s" % (1, 'end'))
        text_widget.delete("1.0", "end")
        error_logs = scripts_and_functions.print_error_log()
        text_widget.insert(index="%d.%d" % (0, 0), chars=error_logs)
        text_widget.insert(index="%d.%d" % (0, 0), chars=error_logs)
    ...
    def error_log(self, resource):
        ...
        if hasattr(self, 'text_snp_debug') and self.text_snp_debug.winfo_exists():
            self.text_snp_debug.delete("1.0", "end")
            self.text_snp_debug.insert(index="%d.%d" % (0, 0), chars=error_output)
        if hasattr(self, 'text_gen_controls_pull_in_debug') and self.text_gen_controls_pull_in_debug.winfo_exists():
            self.text_gen_controls_pull_in_debug.delete("1.0", "end")
