...
from .gui_utils import add_label_frame, add_button, add_label, add_entry, add_combobox, create_canvas, add_slider, update_entries, update_button, filetypes_dir, tab_pad_x
...
class S3pDisplayWindow(ttk.Frame):
    def __init__(self, master, app: "Window"):
        ...
        self.app.entered_var_s3p = add_entry(frame_s3p_dir, text_var=self.app.s3p_dir_name, width=70, col=2,
                                         row=1)
        file_s3p = filetypes_dir(self.app.entered_var_s3p.get())[0]
        self.app.s3p_file_name_combobox = add_combobox(frame_s3p_dir, text=file_s3p, col=2, row=2, width=100)
        ...
        self.app.button_file_update = add_button(tab=frame_s3p_dir, button_name='Update Files',
                                             command=lambda: [
                                                 update_entries(directory=self.app.entered_var_s3p.get(),
                                                                combobox=self.app.s3p_file_name_combobox,
                                                                filetype='.s3p'),
                                                 update_button(self.app.button_file_update)], col=3,
                                             row=1)
        # Adding buttons
        add_button(tab=frame_s3p_dir, button_name='Exit',
                   command=master.destroy, col=5, row=1)
        add_button(tab=frame_s3p_dir, button_name='Plot', command=lambda: self.app.trace_s3p(filename=self.app.s3p_file_name_combobox.get(), s_param=self.app.s_parameter_chosen_s3p.get()),
                   col=3, row=3)
