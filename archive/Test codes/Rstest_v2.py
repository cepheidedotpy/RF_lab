...
def bias_voltage(
        voltage):  # Set bias voltage from user input to correspond to signal generator input. [sig_gen_voltage = user_input/20] because of the amplifier
...
def file_get(filename, ZVA_File_Dir, PC_File_Dir, extension='s2p'):
    ...
    if extension == 's3p':
        try:
            zva.read_file_from_instrument_to_pc("{}\{}.s3p".format(ZVA_File_Dir, filename),
                                                "{}\{}.s3p".format(PC_File_Dir, filename), append_to_pc_file=False)
        except TimeoutException as e:
            print(e.args[0])
    ...
    if extension == 's2p':
        try:
            zva.read_file_from_instrument_to_pc("{}\{}.s2p".format(ZVA_File_Dir, filename),
                                                "{}\{}.s2p".format(PC_File_Dir, filename), append_to_pc_file=False)
        except TimeoutException as e:
            print(e.args[0])
            print('TimeoutException Error \n')
        except StatusException as e:
...
def triggered_data_acquisition(filename=r'default', ZVA_File_Dir=r"C:\Rohde&Schwarz\Nwa\Traces",
                               PC_File_Dir=r"X:\Affaire\Mesure\Mesures Labo RF", file_format='s2p'):
    try:
