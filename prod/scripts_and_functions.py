...
def set_signal_generator_pulse_parameters(pulse_width: float, pulse_period: float, burst_mode: bool):
    """
    ...
    """
    try:
        signal_generator.write(f"SOURce1:FUNCtion:PULSe:WIDTh {pulse_width}US")
        signal_generator.write(f"SOURce1:FUNCtion:PULSe:PERiod {pulse_period}US")
        if burst_mode:
...
def calculate_actuation_and_release_voltages(v_bias, v_logamp, detector_coefficient=1, step=20):
    ...
    if positive_bias.size > 0:
        ...
        min_smoothed_diff_logamp_2 = np.min(
            ...
            first_index_pos[0] + max_positive_bias_index:first_index_pos[0] + len(positive_bias)])
        ...
        try:
            ...
            print('Did not find an index for pull-in voltage using differentiation method (positive bias)')
        ...
        try:
            ...
            print('Did not find an index for pull-out voltage using differentiation method (positive bias)')
    ...
    if negative_bias.size > 0:
        ...
        min_smoothed_diff_logamp_2_neg = np.min(smoothed_diff_logamp_2[
                                                ...
                                                min_negative_bias_index + min_negative_bias_index:first_index_neg[
                                                                                                      0] + len(
        ...
        try:
            ...
            pullout_index_neg = int(np.where(
                ...
                first_index_neg[0] + min_negative_bias_index:last_index_neg[0]] <= min_smoothed_diff_logamp_2_neg)[0][
                                        0])
...
def load_pattern(
    ...
    try:
        ...
        signal_generator.write(
            r'MMEMory:CDIRectory "USB:\PATTERNS\"')  # Set the memory directory to PATTERNS on the USB.
    except:
...
if __name__ == "__main__":
    ...
    signal_generator.write("*RST")
    time.sleep(3)
    load_pattern(r"1000pulses_100us_pulse_dc10_36Vtop40V_triangle.arb")

    time.sleep(3)
    signal_generator.write("*RST")
    load_pattern(r"1000pulses_100us_pulse_dc5_36Vtop40V_triangle.arb")

    time.sleep(3)
    signal_generator.write("*RST")
    load_pattern(r"1000pulses_100us_pulse_dc20_36Vtop40V_triangle.arb")
    # force_trigger_osc()
    ...
    # powermeter = powermeter_init()
    # signal_generator = sig_gen_init()
    # setPowermeterTraceDuration(duration="200E-6", channel='1')
    # setPowermeterTraceDuration(duration="200E-6", channel='2')
    ...
    # np.savetxt(fname='power_meauresment.csv', delimiter=',', newline='\n', header='time,channel_1(dBm),channel_2(dB)',
    #            X=np.hstack((time_values, power_values_1, power_values_2)))
    # signal_generator.close()
    # powermeter.close()
    # ax: plt.Axes
    # fig: plt.Figure
    # fig, ax = plt.subplots(nrows=2, ncols=1, squeeze=True, sharex=True)
    # ax[0].plot(time_values, power_values_1, label=f'$Channel A$ --> SENS 1', color='r')
    # ax[1].plot(time_values, power_values_2, label=f'$Channel B$ --> SENS 2', color='b')
    # # ax[1].set(ylim=[-20, 20])
    # # ax[0].set(ylim=[-20, 20])
    ...
    #
    # plt.show()  # If running interactively03
